...
function loop() {
  ...
  animBlocks.forEach(b => {
    ...
    const ty = cy(b.toRow);
    if (b.y >= ty) {
      b.y = ty; b.vy = -b.vy * .3; b.bounces = (b.bounces || 0) + 1;
      spawnParticles(cx(b.col) + CS / 2, ty + CS, b.pal, 4);
      if (b.bounces >= 3 || Math.abs(b.vy) < 1.2) { b.y = ty; b.vy = 0; b.landed = true; }
    }
...
function touchLiftPx() {
  return activePointerType === 'touch' ? 54 : 0;
}
...
function onPointerUp(e) {
  ...
  setTimeout(() => { if (dragPieceIdx >= 0) endDrag(); }, 60);
}
